...
export default function App() {
  ...
  const [deadlineSaved, setDeadlineSaved] = useState(
    localStorage.getItem("dl_order_deadline") || DEFAULT_DEADLINE
  );
  ...
  const deadlineTime = new Date(deadlineSaved).getTime();
  ...
  const deadlineDisplay = new Date(deadlineSaved).toLocaleString("de-DE", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  function savePasswords(e) {
    ...
    if (deadlineInput) {
      localStorage.setItem("dl_order_deadline", deadlineInput);
      setDeadlineSaved(deadlineInput);
    }

    setNewMemberPassword("");
    setNewAdminPassword("");
    setSettingsMessage("Einstellungen wurden aktualisiert.");
