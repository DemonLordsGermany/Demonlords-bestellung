              ...
              <div className="tableWrap">
                <table>
                  ...
                  <tbody>
                    {orders.length === 0 && (
                      <tr>
                        <td colSpan={isAdmin ? 16 : 3} className="empty">Noch keine Bestellungen eingetragen.</td>
                      </tr>
                    )}
                      ...
                      <tr key={o.id}>
                        {isAdmin ? (
                          <>
                            ...
                            <td className="noteCell">{o.note || "-"}</td>
...
.page{min-height:100dvh;background:#000;padding:clamp(4px,1vw,10px)}
.frame{width:100%;max-width:1600px;min-height:calc(100dvh - clamp(8px,2vw,20px));margin:0 auto;border:1px solid #8f0907;border-radius:clamp(4px,.6vw,8px);background:#030303;overflow:hidden;box-shadow:0 0 38px rgba(150,0,0,.42);display:flex;flex-direction:column}

.hero{position:relative;height:clamp(132px,17dvh,170px);min-height:132px;overflow:hidden;background:radial-gradient(circle at 50% 18%,#252525 0%,#101010 34%,#050505 62%,#000 100%);border-bottom:1px solid #240000;flex:0 0 auto}
...
.deadlineHeader{position:absolute;left:50%;bottom:clamp(8px,1.1vw,12px);transform:translateX(-50%);z-index:5;border:1px solid #8b0000;background:rgba(0,0,0,.75);color:#fff;border-radius:5px;padding:clamp(6px,.8vw,8px) clamp(10px,1vw,16px);font-family:'Oswald',Arial,sans-serif;font-weight:700;letter-spacing:.08em;box-shadow:0 0 12px rgba(255,0,0,.25);font-size:clamp(11px,.9vw,14px);white-space:nowrap}.deadlineHeader.closed{color:#ff1c15;border-color:#ff1c15}
...
.headline h2{margin:4px 0 0;font-family:'Rye',Georgia,serif;font-size:clamp(22px,3vw,38px);line-height:1;letter-spacing:.2em;color:#d8d8d8;text-shadow:0 2px 0 #000,0 0 6px rgba(255,255,255,.14);font-weight:400;white-space:nowrap}
.headline p{display:inline-block;margin:clamp(6px,1vw,10px) 0 0;padding-top:clamp(5px,.8vw,8px);min-width:min(330px,100%);border-top:1px solid #8b0000;font-family:'Rye',Georgia,serif;font-size:clamp(13px,1.6vw,20px);letter-spacing:.18em;color:#cfcfcf;text-shadow:0 2px 0 #000;white-space:nowrap}

.content{display:grid;grid-template-columns:minmax(350px,420px) minmax(0,1fr);gap:clamp(8px,1vw,14px);padding:0 clamp(8px,1.2vw,18px) clamp(8px,1vw,14px);flex:1;min-height:0}
...
.formPanel{padding:clamp(10px,1vw,15px);height:100%;overflow:auto;backdrop-filter:blur(3px)}.overviewPanel{overflow:hidden;display:flex;flex-direction:column;min-width:0;height:100%}

.redTitle{margin:0;color:#ef1b16;font-size:clamp(18px,1.5vw,22px);font-weight:700;letter-spacing:.035em;text-transform:uppercase;text-shadow:0 0 8px rgba(255,0,0,.25)}
.smallText{margin:5px 0 clamp(10px,1vw,16px) clamp(20px,2vw,38px);color:#e2e2e2;font-size:clamp(12px,.95vw,14px);font-weight:300;letter-spacing:.02em}
form{display:flex;flex-direction:column;gap:clamp(10px,1vw,15px)}.twoCols{display:grid;grid-template-columns:1fr 1fr;gap:clamp(8px,1vw,13px)}
label{display:block;font-size:clamp(13px,.95vw,14px);font-weight:400;color:#f1f1f1;letter-spacing:.025em}label span{color:#ff2018}
input,select,textarea{width:100%;margin-top:5px;height:clamp(34px,3.8dvh,38px);border-radius:5px;border:1px solid #444;background:linear-gradient(180deg,#090909,#030303);color:#f3f3f3;padding:0 10px;outline:none;font-family:'Oswald',Arial,sans-serif;font-size:clamp(14px,1vw,15px)}
input:focus,select:focus,textarea:focus{border-color:#c40000;box-shadow:0 0 8px rgba(220,0,0,.45)}
textarea{height:clamp(38px,5dvh,44px);padding-top:9px;resize:vertical;min-height:38px;max-height:100px}
.product h4{margin:0 0 8px;font-size:clamp(17px,1.3vw,19px);font-weight:700;text-transform:uppercase;letter-spacing:.02em;color:#e9e9e9}.product small{font-size:13px;color:#dedede;text-transform:none;font-weight:400}
.shirtGrid{display:grid;grid-template-columns:1fr 132px 58px;gap:clamp(8px,.9vw,11px);align-items:end}.twoProductCols{display:grid;grid-template-columns:1fr 66px;gap:clamp(12px,1.5vw,22px)}
.sizeRow{display:flex;gap:5px;flex-wrap:wrap;margin-top:6px}.sizeRow span{min-width:28px;text-align:center;color:#e7e7e7;border:1px solid #343434;border-radius:4px;background:rgba(0,0,0,.6);padding:1px 5px;font-size:12px}
.submit{height:clamp(40px,4dvh,44px);border:0;border-radius:5px;background:linear-gradient(180deg,#fa2a23,#b70d09);color:#fff;font-family:'Oswald',Arial,sans-serif;font-size:clamp(16px,1.2vw,18px);font-weight:700;text-transform:uppercase;letter-spacing:.06em;cursor:pointer;box-shadow:0 0 16px rgba(255,0,0,.45)}
.submit:hover,.export:hover,.delete:hover,.memberLogout:hover,.loginBox button:hover{filter:brightness(1.15)}.privacy{margin:-5px 0 0;text-align:center;color:#aaa;font-size:12px;font-weight:300}.closedNotice{border:1px solid #8b0000;background:rgba(120,0,0,.22);color:#ffb7b7;border-radius:5px;padding:12px;text-align:center;font-weight:700;text-transform:uppercase}.memberTotalBox{border:1px solid #8b0000;background:rgba(120,0,0,.18);border-radius:6px;padding:10px 12px;display:flex;justify-content:space-between;align-items:center;gap:12px}.memberTotalBox span{color:#ddd;font-size:14px}.memberTotalBox strong{color:#ff2119;font-size:clamp(20px,1.8vw,24px)}

.overviewHead{display:flex;justify-content:space-between;gap:12px;padding:clamp(10px,1vw,14px) clamp(10px,1.2vw,16px);border-bottom:1px solid #272727;flex:0 0 auto}.adminBox{display:flex;align-items:flex-start;justify-content:flex-end}.adminForm{display:grid;grid-template-columns:150px auto;gap:8px;align-items:start}.adminForm input{height:38px;margin:0}.adminForm button,.logout{height:38px;border:1px solid #8b0000;background:#070707;color:#eee;border-radius:5px;padding:0 12px;font-family:'Oswald',Arial,sans-serif;font-size:13px;text-transform:uppercase;cursor:pointer}.adminForm span{grid-column:1/3;color:#ff1c15;font-size:12px;text-align:right}.adminActive{display:flex;gap:8px;align-items:center}.logout{border-color:#555;color:#bbb}
.export{align-self:start;border:1px solid #8b0000;background:#070707;color:#eee;border-radius:5px;padding:9px 15px;font-family:'Oswald',Arial,sans-serif;font-size:14px;text-transform:uppercase;cursor:pointer;letter-spacing:.03em}
.passwordPanel{border-bottom:1px solid #242424;padding:10px 16px;background:rgba(0,0,0,.35);gap:8px}.passwordPanel h3{margin:0;color:#ef1b16;font-size:17px;text-transform:uppercase}.passwordPanel p{margin:4px 0 0;color:#55ff7a}.passwordGrid{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px;align-items:end}.passwordGrid button{height:36px;border:1px solid #8b0000;background:#070707;color:#eee;border-radius:5px;padding:0 14px;font-family:'Oswald',Arial,sans-serif;text-transform:uppercase;cursor:pointer}
.tableWrap{overflow:auto;flex:1;min-height:180px;max-height:48dvh}table{width:100%;min-width:0;border-collapse:collapse;font-size:clamp(12px,.85vw,13px);table-layout:auto}th,td{padding:clamp(6px,.7vw,8px);border-bottom:1px solid #242424;text-align:left;white-space:nowrap}th{color:#f5f5f5;font-weight:600;letter-spacing:.02em}th[colspan]{text-align:center}tr:hover td{background:rgba(255,255,255,.035)}.price{color:#ff1610;font-weight:700}.noteCell{max-width:220px;white-space:normal;color:#ddd;line-height:1.3}.memberStatus{color:#55ff7a;font-weight:700}.delete{border:1px solid #b00000;color:#ff1610;background:transparent;border-radius:5px;padding:5px 7px;cursor:pointer}.empty{text-align:center;color:#aaa;padding:28px}
.bottomCards{display:grid;grid-template-columns:1.15fr .95fr;gap:clamp(8px,1vw,14px);padding:clamp(8px,1vw,14px);flex:0 0 auto}.overviewPanel:not(.adminView) .bottomCards{display:none}.card{border:1px solid #505050;border-radius:8px;background:rgba(0,0,0,.55);padding:clamp(12px,1.2vw,18px) clamp(14px,1.7vw,24px);box-shadow:inset 0 0 28px rgba(255,255,255,.035)}.card h3{margin:0 0 12px;text-align:center;color:#f01b15;font-size:clamp(18px,1.5vw,22px);font-weight:700;text-transform:uppercase;letter-spacing:.03em}.summaryLine{display:grid;grid-template-columns:34px 1fr 64px 100px;align-items:center;gap:6px;margin-bottom:6px;font-size:clamp(15px,1.1vw,18px)}.summaryLine span,.priceLine span{font-size:clamp(22px,2vw,28px);filter:saturate(1.25)}.summaryLine em,.priceLine em{font-style:normal}.total{display:flex;justify-content:space-between;margin-top:10px;padding-top:10px;border-top:1px solid #555;color:#f01b15;font-size:clamp(22px,1.8vw,26px);font-weight:700}.priceLine{display:grid;grid-template-columns:36px 1fr auto;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid #333;font-size:clamp(15px,1.1vw,18px)}.priceLine:last-child{border-bottom:0}
footer{margin:0 clamp(8px,1.2vw,18px) clamp(8px,1vw,14px);border:1px solid #242424;border-radius:8px;background:#050505;color:#d0d0d0;text-align:center;padding:10px;font-family:'Rye',Georgia,serif;font-size:clamp(9px,.85vw,12px);letter-spacing:.18em;text-transform:uppercase;flex:0 0 auto}
